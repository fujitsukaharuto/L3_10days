...
OutlinePipe::~OutlinePipe() {}


void OutlinePipe::CreateRootSignature(ID3D12Device* device) {
...
void OutlinePipe::CreatePSO(ID3D12Device* device) {
	...
	assert(vs != nullptr);
	ps = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"DepthBasedOutline.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
