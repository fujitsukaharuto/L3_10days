...
GrayPipeline::~GrayPipeline() {}


void GrayPipeline::CreateRootSignature(ID3D12Device* device) {
...
void GrayPipeline::CreatePSO(ID3D12Device* device) {
	...
	assert(vs != nullptr);
	ps = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"SimpleGrayscale.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
