...
GaussCSPipe::~GaussCSPipe() {
}

void GaussCSPipe::CreateRootSignature(ID3D12Device* device) {
    HRESULT hr;
...
void GaussCSPipe::CreatePSO(ID3D12Device* device) {
    HRESULT hr;


    vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"CS/Gaussian.CS.hlsl", L"cs_6_0");
    assert(vs != nullptr);
