...
RadialBlurCSPipe::~RadialBlurCSPipe() {
}

void RadialBlurCSPipe::CreateRootSignature(ID3D12Device* device) {
    HRESULT hr;
    ...
    rootParameters[2].Descriptor.ShaderRegister = 0; // b0 レジスタ
...
void RadialBlurCSPipe::CreatePSO(ID3D12Device* device) {
    HRESULT hr;


    vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"CS/RadialBlur.CS.hlsl", L"cs_6_0");
    assert(vs != nullptr);
