...
BloomCSPipe::~BloomCSPipe() {
}

void BloomCSPipe::CreateRootSignature(ID3D12Device* device) {
    HRESULT hr;
...
void BloomCSPipe::CreatePSO(ID3D12Device* device) {
    HRESULT hr;


    vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"CS/Bloom.CS.hlsl", L"cs_6_0");
    assert(vs != nullptr);
