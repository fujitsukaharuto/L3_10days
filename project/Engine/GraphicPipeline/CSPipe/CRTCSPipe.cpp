...
void CRTCSPipe::CreateRootSignature(ID3D12Device* device) {
    HRESULT hr;
...
void CRTCSPipe::CreatePSO(ID3D12Device* device) {
    HRESULT hr;


    vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"CS/CRTEffect.CS.hlsl", L"cs_6_0");
    assert(vs != nullptr);
