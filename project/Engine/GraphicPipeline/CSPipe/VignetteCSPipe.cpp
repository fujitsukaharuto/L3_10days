...
VignetteCSPipe::~VignetteCSPipe() {
}

void VignetteCSPipe::CreateRootSignature(ID3D12Device* device) {
    HRESULT hr;
...
void VignetteCSPipe::CreatePSO(ID3D12Device* device) {
    HRESULT hr;


    vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"CS/Vignette.CS.hlsl", L"cs_6_0");
    assert(vs != nullptr);
