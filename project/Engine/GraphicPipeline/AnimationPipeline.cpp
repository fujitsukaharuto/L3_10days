...
void AnimationPipeline::CreateRootSignature(ID3D12Device* device) {
	...
	rootParameters[7].DescriptorTable.pDescriptorRanges = &descriptorRange[0];
...
void AnimationPipeline::CreatePSO(ID3D12Device* device) {
	...
	vs = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"SkinningObject3d.VS.hlsl", L"vs_6_0");
	assert(vs != nullptr);
	ps = dxcommon_->GetDXCompil()->CompileShader(kDirectoryPath_ + L"SkinningObject3d.PS.hlsl", L"ps_6_0");
	assert(ps != nullptr);
